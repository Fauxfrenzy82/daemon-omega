...
/**
 * Check if any strategy is enabled (only rateArb and vaultArb are active)
 */
function isAnyStrategyEnabled(): boolean {
  if (!env.MASTER_STRATEGY_ENABLED) return false;
  return (
    env.STRATEGY_RATE_ARB_ENABLED ||
    env.STRATEGY_VAULT_ARB_ENABLED
  );
}

async function bootstrap(): Promise<void> {
  log.info('Starting Chronos/Enso arbitrage system (Rate Arbitrage + Vault Arbitrage only)', {
    env: env.NODE_ENV,
    ...
    rateArbEnabled: env.STRATEGY_RATE_ARB_ENABLED,
  ...
  // ✅ Step 2: No protocol discovery needed – we only use Enso routes and direct queries
  ...
  log.info('✅ System running with Rate Arbitrage + Vault Arbitrage');
}
