...
import { RewardPosition } from './farms';
...
async function findGammaFarmForPool(
  ...
  try {
    // The Gamma Hypervisor address is often derived from the pool
    // In QuickSwap V3, Gamma Hypervisors are deployed for each pool
    // The pattern is: Gamma deploys a Hypervisor contract that wraps the pool
    
    // Attempt to find the Hypervisor via known patterns
    // For now, we check if the pool itself supports the Hypervisor interface
    
    const hypervisor = new ethers.Contract(poolAddress, HYPERVISOR_ABI, provider);
    
    // Check if this is a Gamma Hypervisor by calling totalSupply
    try {
      ...
      if (totalSupply && totalSupply.gt(0)) {
        ...
        return {
          ...
          farmAddress: null, // The Hypervisor itself acts as the farm
        };
      }
    } catch (err) {
      // Not a Hypervisor
...
export async function discoverGammaFarms(): Promise<Record<string, string>> {
  ...
  try {
    ...
    for (const [pairId, config] of Object.entries(TARGET_PAIRS)) {
      log.debug(`Checking pair: ${pairId}`);
      ...
      if (!poolAddress) {
        log.debug(`No pool found for ${pairId}`);
        continue;
      }
      
      log.debug(`Pool found for ${pairId}: ${poolAddress}`);
      
      // Try to find a Gamma farm for this pool
      const farmInfo = await findGammaFarmForPool(poolAddress);
      
      if (farmInfo.hypervisorAddress) {
        farms[pairId] = farmInfo.hypervisorAddress;
        log.info(`✅ Found Gamma farm for ${pairId}`, {
          address: farmInfo.hypervisorAddress,
        });
      } else {
        log.debug(`No active Gamma farm found for ${pairId}`);
      }
    }
  } catch (err) {
    log.error('Failed to discover Gamma farms', {
 ...
 */
export async function generateRewardPositions(): Promise<RewardPosition[]> {
  ...
  const beefyAddress = process.env.BEEFY_VAULT_ADDRESS;
  ...
  const balancerGauge = process.env.BALANCER_GAUGE_ADDRESS;
